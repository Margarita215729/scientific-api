...
# Скрипт для сборки и развертывания AMD64-совместимого образа в Azure
# Решает проблему с несовместимостью ARM64 образа в Azure Container Instances
...
CONTAINER_NAME="scientific-api-amd64"
LOCATION="eastus"
ACR_NAME="scientificapiacr"
ACR_IMAGE="${ACR_NAME}.azurecr.io/scientific-api-amd64:latest"

echo -e "${GREEN}=== Сборка и развертывание AMD64-совместимого образа в Azure ===${NC}"
...
# Шаг 2: Создание временного файла main.py
echo -e "${BLUE}Шаг 2: Создание временного файла api/main.py...${NC}"
cat > api/main.py << 'EOF'
...
echo -e "${GREEN}Временный файл api/main.py создан.${NC}"

# Шаг 3: Сборка Docker образа для AMD64
echo -e "${BLUE}Шаг 3: Сборка Docker образа для AMD64 архитектуры...${NC}"
docker buildx build --platform linux/amd64 -f Dockerfile.azure -t scientific-api-amd64:latest .
IMAGE_ID=$(docker images scientific-api-amd64:latest --quiet)
...
az container create \
    ...
    --dns-name-label scientific-api-amd64-$(date +%s) \
    --ports 8000 \
...
echo ""
echo -e "${GREEN}🎉 Scientific API успешно развернут в Azure!${NC}"
...
echo -e "${YELLOW}Удалить образ из ACR:${NC} az acr repository delete --name $ACR_NAME --image scientific-api-amd64:latest --yes"
