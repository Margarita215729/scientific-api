...
# Простое развертывание Scientific API
# Использует Ubuntu образ с установкой Python
...
CONTAINER_NAME="scientific-api-simple"

echo -e "${GREEN}=== Простое развертывание Scientific API ===${NC}"
...
az container create \
    ...
    --cpu 4 \
    --memory 8 \
    --dns-name-label scientific-simple-$(date +%s) \
    --ports 8000 \
    ...
    --command-line "sh -c 'apk add --no-cache python3 py3-pip && pip3 install requests fastapi uvicorn && python3 -c \"
...
# Тестирование
echo -e "${YELLOW}Тестируем API (может занять до 120 секунд)...${NC}"

for i in {1..24}; do
    echo -e "${YELLOW}Попытка $i/24 - ждем запуска...${NC}"
